...
from utils.jittered_center_crop import jittered_center_crop
...
class TransTProcessing(BaseProcessing):
    ...
    def __call__(self,data:TensorDict):
        ...
        for s in ['search', 'template']:
            ...
            if s == 'search':
                crops, boxes, _ = jittered_center_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
            ...
            try:
                ...
                data[s + '_images'] = self.transform[s](crops)
                data[s + '_anno']=boxes
        # Prepare output
