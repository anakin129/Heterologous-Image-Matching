...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,args,
    ...
    def __getitem__(self,index):
        ...
        """
        #select a dataset
        dataset=random.choices(self.datasets,weights=self.p_datasets)[0] #the return of random.choices is a list
        ...
        if is_video_dataset:
            ...
            if self.frame_sample_mode=='interval':
                ...
                while search_frame_ids is None:
                    ...
                    extra_template_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,\
                        min_id=base_frame_id[0] - self.max_gap - gap_increase,max_id=base_frame_id[0] + self.max_gap + gap_increase)
            ...
            template_frame_ids = [1] * self.num_template_frames  #[1,1,..]
            search_frame_ids = [1] * self.num_search_frames
        
        if dataset.get_name()=='m3fd' or dataset.get_name()=='kaist':
            if not self.image_source:
                template_frames, template_anno, meta_obj_template = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict,'vis')
                search_frames, search_anno, meta_obj_search = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict,'lwir')
            else:
                template_frames, template_anno, meta_obj_template = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict,'vis')
                search_frames, search_anno, meta_obj_search = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict,'vis')
        else:
...
class TransTSampler(TrackingSampler):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames=1, num_template_frames=1, processing=no_processing, frame_sample_mode='interval',args=None):
        super().__init__(datasets=datasets, p_datasets=p_datasets, samples_per_epoch=samples_per_epoch, max_gap=max_gap,
                         ...
                         frame_sample_mode=frame_sample_mode,args=args)