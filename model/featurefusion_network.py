...
class FeatureFusionNetwork(nn.Module): 
    ...
    def __init__(self, args,d_model=512, nhead=8, num_featurefusion_layers=4,
                 dim_feedforward=2048, dropout=0.1, activation="relu"):
        super().__init__()

        featurefusion_layer = FeatureFusionLayer(args,d_model, nhead, dim_feedforward, dropout, activation)
        if args.fusion_network_id==1:
            self.encoder = Encoder(featurefusion_layer, num_featurefusion_layers+1)
        else:
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        src_temp = src_temp.flatten(2).permute(2, 0, 1)  # 8x256x16x16-->8x256x256-->256x8x256
        pos_temp = pos_temp.flatten(2).permute(2, 0, 1)  # 8x256x16x16-->8x256x256-->256x8x256
        src_search = src_search.flatten(2).permute(2, 0, 1)
        pos_search = pos_search.flatten(2).permute(2, 0, 1)
        mask_temp = mask_temp.flatten(1)   # 8x16x16-->8x256
        ...
        hs = self.decoder(memory_search, memory_temp,
         ...
         #hs : 1024 x 8 x 256
        return hs.unsqueeze(0).transpose(1, 2),memory_temp.unsqueeze(0).transpose(1, 2),memory_search.unsqueeze(0).transpose(1, 2)
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
                     ...
                     pos_src1: Optional[Tensor] = None,
                     pos_src2: Optional[Tensor] = None):
        ## src1.shape,src2.shape,src1_key_padding_mask.shape,src2_key_padding_mask.shape, pos_src1.shape, pos_src2.shape)
        ## torch.Size([256, 8, 256]) torch.Size([1024, 8, 256]) torch.Size([8, 256]) torch.Size([8, 1024]) torch.Size([256, 8, 256]) torch.Size([1024, 8, 256])
        ## src1_mask=None,src2_mask=None
        q1 = k1 = self.with_pos_embed(src1, pos_src1)  # 256 * 8 *256
        ...
        if self.args.fusion_network_id==0:
...
def build_featurefusion_network(args,num=cfg.TRANST.featurefusion_layers):
    return FeatureFusionNetwork(
        d_model=cfg.TRANST.hidden_dim,
        dropout=cfg.TRANST.dropout,
        nhead=cfg.TRANST.nheads,
        dim_feedforward=cfg.TRANST.dim_feedforward,
        num_featurefusion_layers=num,
        args=args
    )
