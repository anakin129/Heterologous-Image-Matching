...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, output_layers, num_classes=1000, inplanes=64, dilation_factor=1):
        ...
        if len(output_layers)==2:
            self.proj=nn.Conv2d(512,1024,kernel_size=1,stride=1,padding=0)
    ...
    def forward(self, x, output_layers=None):
        ...
        if self.layer3 is not None:
            ...
            if self._add_output_and_check('layer3', x, outputs, output_layers):
                if len(output_layers)==2:
                    output={}
                    output['layer_multi']=self.proj(outputs['layer2'])+outputs['layer3']
                    outputs=output
                return outputs
...
def resnet50(output_layers=None, pretrained=False,vis=1,**kwargs):
    ...
    if vis==1:
        model = ResNet(Bottleneck, [3, 4, 6], output_layers, **kwargs)
    else:
        model = ResNet(Bottleneck, [3], output_layers, **kwargs)
    if pretrained:
